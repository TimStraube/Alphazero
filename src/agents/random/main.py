    ...
    def run(self):
        ...
        for i in range(self.episodes):
            ...
            if self.writer is not None:
                try:
                    ...
                    # running mean of episode lengths over last `window` episodes (match PPO logging)
                    try:
                        mean_recent = float(np.mean(lengths[-self.window:]))
                    except Exception:
                        mean_recent = float(avg_so_far)
                    self.writer.add_scalar("train/ep_len_mean", mean_recent, i)
                except Exception:
                    pass
        ...
        if self.writer is not None:
            try:
                ...
                # persist final ep_len_mean
                try:
                    final_mean = float(np.mean(lengths[-self.window:])) if lengths else 0.0
                except Exception:
                    final_mean = float(avg)
                self.writer.add_scalar("train/ep_len_mean", final_mean, len(lengths)-1 if lengths else 0)
                self.writer.flush()
            except Exception:
                pass
        return avg